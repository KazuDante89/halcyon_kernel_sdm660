...
function finerr() {
    curl -s -X POST "https://api.telegram.org/bot$token/sendMessage" \
        ...
        -d text="Build failed due to errors that occured"
        -d text="Well that sucks fuzzy monkey balls"
    exit 1
}
# Compile plox
function compile() {
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 [EAS-WALT]-Nebula-Kernel_v0.0.0.zip *
    cd ..
}
sendinfo
compile
